...
public class CommandSession {
    ...
    public boolean prepareSession(ICatchITransport itrans,boolean enablePTPIP) {
        ...
        try {
            AppLog.d(tag, "begin setConnectionCheckParam connectionCheckTimes:" + AppInfo.connectionCheckTimes + " connectionCheckInterval:" + AppInfo.connectionCheckInterval);
            boolean ret = ICatchCameraSession.getCameraConfig(itrans).setConnectionCheckParam(AppInfo.connectionCheckTimes, AppInfo.connectionCheckInterval,0);// var1:检测次数; var2:检测间隔,单位s
            AppLog.d(tag, "end setConnectionCheckParam ret:" + ret);
        } catch (IchInvalidArgumentException e) {
            e.printStackTrace();
        }
        sessionPrepared = true;
    ...
    public ICatchCameraSession getSession() {
