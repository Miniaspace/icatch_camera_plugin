...
import com.icatch.mobilecam.utils.fileutils.StorageManage;
...
//            }
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                String cachePath = context.getExternalCacheDir().getPath() + AppInfo.DOWNLOAD_PATH;
                File cacheDirFile = new File(cachePath);
                if (!cacheDirFile.exists()) {
                    cacheDirFile.mkdirs();
                }
                curFilePath = cachePath + fileName;
            }else {
            ...
            if (retvalue) {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
//                    Uri uri = StorageManage.getExternalNewMediaFileUri(context, "DCIM/Mobiletest", fileName);
//                    StorageManage.copySandFileToExternalUri(context, curFilePath, uri);
                    String localPath = "";
                    if (downloadFile.getFileType() == ICatchFileType.ICH_FILE_TYPE_IMAGE) {
                        localPath = AppInfo.DOWNLOAD_PATH_PHOTO;
                    } else {
                        localPath = AppInfo.DOWNLOAD_PATH_VIDEO;
                    }
                    FileUtil.copyPrivateToDCIM(context, curFilePath, localPath, fileName, true);
                }else {
