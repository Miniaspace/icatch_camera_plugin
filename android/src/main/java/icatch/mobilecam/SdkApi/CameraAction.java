...
public class CameraAction {
    ...
    public boolean stopVideoCapture() {
        boolean retVal = false;
        int timeout = 8;//Units are seconds
        AppLog.i(TAG, "begin stopVideoCapture timeout:" +timeout);
        try {
            int ret = cameraControl.stopMovieRecord(timeout);
            if(ret == 0){
                retVal = true;
            }
        } catch (IchSocketException e) {
    ...
    public boolean formatStorage() {
        ...
        int timeout = 15;//Units are seconds
        try {
            int ret = cameraControl.formatStorage(timeout);
            if(ret == 0){
                retVal = true;
            }
        } catch (IchSocketException e) {
    ...
    public boolean updateFW(String fileName) {
        boolean ret = false;
        AppLog.i(TAG, "begin update FW:" + fileName);
        CommandSession mSDKSession = CameraManager.getInstance().getCurCamera().getSDKsession();
        try {
            ret = cameraAssist.updateFw(mSDKSession.getSession(), fileName);
        } catch (IchInvalidSessionException e) {
            // TODO Auto-generated catch block
            AppLog.e(TAG, "IchInvalidSessionException");
            e.printStackTrace();
