...
import com.icatch.mobilecam.Log.BaseLog;
...
public class ConfigureInfo {
    ...
    private final String[] cfgTopic = {
            ...
            "connectionCheckInterval=4",
            "connectionCheckTimes=4",
            "localPbUseSdkRender=false"
    };
    ...
    public void initCfgInfo(Context context) {
        ...
        String connectionCheckTimes = null;
        String localPbUseSdkRender = null;
        try {
            ...
            connectionCheckInterval = cfgInfo.getProperty("connectionCheckInterval");
            connectionCheckTimes = cfgInfo.getProperty("connectionCheckTimes");
            localPbUseSdkRender = cfgInfo.getProperty("localPbUseSdkRender");
        } catch (FileNotFoundException e) {
            // TODO Auto-generated catch block
            e.printStackTrace();
        } catch (IOException e) {
...
//                MediaLog.enableAppLog(context);  // disable medialog
                com.icatch.mobilecam.Log.AppLog.enableAppLog(context);
            }
        ...
        if(localPbUseSdkRender != null && localPbUseSdkRender.equals("true")){
            AppInfo.localPbUseSdkRender = true;
        }


        AppLog.d(TAG, "readCfgInfo connectionCheckTimes=" + connectionCheckTimes + " connectionCheckInterval=" + connectionCheckInterval);

        if (connectionCheckTimes != null) {
            try {
                AppInfo.connectionCheckTimes  = Integer.parseInt(connectionCheckTimes);
            }catch (Exception e){
                AppLog.d(TAG, "readCfgInfo connectionCheckTimes parseInt Exception:" + e.getMessage());
            }
        }

        if (connectionCheckInterval != null) {
            try {
                AppInfo.connectionCheckInterval  = Integer.parseInt(connectionCheckInterval);
            }catch (Exception e){
                AppLog.d(TAG, "readCfgInfo connectionCheckInterval parseInt Exception:" + e.getMessage());
            }
        }
