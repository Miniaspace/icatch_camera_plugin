...
//            AppLog.d(TAG,"onPageScrolled arg0:" + arg0 + " arg1:" + arg1 + " arg2:" + arg2);
    ...
    private void updateUi() {
        ...
        FileItemInfo itemInfo = fileList.get(curIndex);
    ...
    private void loadPanoramaPhoto(final FileItemInfo itemInfo) {
        if (itemInfo == null) {
            return;
        }
        boolean isPanorama = PanoramaTools.isPanorama(itemInfo.getFilePath());
        if (isPanorama) {
            ImageLoaderUtil.loadLocalImage(new File(itemInfo.getFilePath()), new ImageLoaderUtil.OnLoadListener() {
                @Override
    ...
    public void share(Context context) {
        ...
        String photoPath = fileList.get(curPosition).getFilePath();
    ...
    private class DeleteThread implements Runnable {
        ...
        public void run() {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                FileItemInfo tempFile = fileList.get(curPhotoIdx);
                int retInt = FileUtil.deleteFile(activity, tempFile.getUri());
                AppLog.d(TAG, "FileUtil.deleteFile: " + tempFile.getFileName() + ", ret: " + retInt);
            }else {
                File tempFile = new File(fileList.get(curPhotoIdx).getFilePath());
                if (tempFile.exists()) {
                    tempFile.delete();
...
//        removeGyroscopeListener();
    ...
    public void onSensorChanged(SensorEvent event) {
        if(fileList== null || curPhotoIdx >= fileList.size()){
            return;
        }
        if(!fileList.get(curPhotoIdx).isPanorama()){
            return;
