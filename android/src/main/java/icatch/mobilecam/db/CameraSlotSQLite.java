...
public class CameraSlotSQLite {
    ...
    public boolean insert(CameraSlot camSlot) {
        ...
        cValue.put("addType", camSlot.addType);
        cValue.put("wifiPassword", camSlot.wifiPassword);
    ...
    public void update(CameraSlot camSlot) {
        ...
        values.put("addType", camSlot.addType);
        values.put("wifiPassword", camSlot.wifiPassword);
    ...
    public void deleteByPosition(int slotPosition) {
        ...
        if(slotPosition >= 0){
            update(new CameraSlot(slotPosition, false, null, CameraType.UNDEFIND_CAMERA, null, false, CameraAddType.DEFAULT, ""));
        }
    ...
    public ArrayList<CameraSlot> getAllCameraSlotFormDb() {
        ...
        while (cursor.moveToNext()) {
            ...
            int addType = cursor.getInt(cursor.getColumnIndex("addType"));
            String wifiPassword = cursor.getString(cursor.getColumnIndex("wifiPassword"));
            camSlotList.add(new CameraSlot(id, switchIntToBool(isUsed), camName, cameraType, imageBuf, isReady, addType, wifiPassword));
        ...
        if (camSlotList.size() == 0) {
            for (int ii = 0; ii < 3; ii++) {
                if (insert(new CameraSlot(ii, false, null, null))) {
                    camSlotList.add(new CameraSlot(ii, false, null, CameraType.UNDEFIND_CAMERA, null, false, CameraAddType.DEFAULT, ""));
                }
            }
        }
